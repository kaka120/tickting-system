const LoginRegistrationController = require('./LoginRegistrationController');
const AuthenticationController = require('./AuthenticationController');
const TicketRegistrationRegistrationController = require('./TicketRegistrationRegistrationController')
const ViewTicketsController = require('./ViewRegistrationRegistrationController');
const ViewTicketByIdController = require('./ViewTicketByIdController');
const UpdateTicketByIdController = require('./UpdateTicketByIdController');
const PostChatWithUserIdController = require('./PostChatWithUserIdController');
const ViewChatHistoryByIdController = require('./ViewVhatHistoryByIdController');

const TeamRegistrationController = require('./TeamRegistrationController');
const ViewTeamController = require('./ViewTeamController');
const EditTeamController = require('./EditTeamController');

const TeamTicketAssignment = require('./TeamTicketAssignment');
const TicketToTeamAssignment = require('./TicketToTeamAssignmentController');

module.exports = {
    LoginRegistrationController,
    AuthenticationController,
    TicketRegistrationRegistrationController,
    ViewTicketsController,
    ViewTicketByIdController,
    UpdateTicketByIdController,
    PostChatWithUserIdController,
    ViewChatHistoryByIdController,
    TeamRegistrationController,
    ViewTeamController,
    EditTeamController,
    TeamTicketAssignment,
    TicketToTeamAssignment
};
